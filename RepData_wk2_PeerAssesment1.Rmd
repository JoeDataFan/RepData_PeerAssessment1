...
```
The mean of total steps per day was `r round(mean_total_steps)` and the median of total steps per day was `r median_total_steps`. This data indicates that the data may be skewed to the left by low step values possibly NAs. 
...
```
Interval `r max_mean_int_steps[1,1]` showed the highest mean number of steps.
...
```
Total number of NAs found within steps variable = `r sum_steps_NA`
...
This particular individual seemed to be more active during the weekend than during the weekdays. Calculating the overall mean of mean steps per 5 minute interval we see that during the week `r round(mean_steps_weekday[1,2])` steps were taken on average in a 5 minute time period compared to `r round(mean_steps_weekend[1,2])` steps during the weekend. 
